...
namespace zuoanqh.libzut.FileIO
{
  ...
  public static class ByLineFileIO
  {
    ...
    {
      using (StreamWriter writer = new StreamWriter(File.OpenWrite(zuio.ToAbsolutePath(fPath)), encoding))
      {
    ...
    public static List<string> ReadFile(string fPath, bool ignoreSpace, bool ignoreEmptyLine, Encoding encoding)
    {
      ...
      using (StreamReader reader = new StreamReader(File.OpenRead(zuio.ToAbsolutePath(fPath)), encoding))
      {
