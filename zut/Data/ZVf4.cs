...
namespace cn.zuoanqh.open.zut.Data
{
  public class zvf4
  {
    ...
    private zvf4(params float[] Data)
    { this.data = Data; }
    public zvf4(float first, float second, float third, float forth)
      : this(new float[] { first, second, third, forth }) { }
    ...
    public static implicit operator zvf4(zvf vec)
    {
      if (vec.Length != 4) throw new InvalidCastException("Given vector's length is not 4.");
      return new zvf4(vec[0], vec[1], vec[2], vec[3]);
    }
    ...
    public static implicit operator zvf(zvf4 vec)
    ...
    /// <returns></returns>
    public static zvf4 operator +(zvf4 op1, zvf4 op2)
    {
      float[] data = new float[op1.Length];
      for (int i = 0; i < op1.Length; i++)
        data[i] = op1.data[i] + op2.data[i];
      return new zvf4(data);
    }
    ...
    /// <param name="s"></param>
    /// <returns></returns>
    public static zvf4 operator *(zvf4 op1, float s)
    {
      float[] data = new float[op1.Length];
      for (int i = 0; i < op1.Length; i++)
        data[i] = op1.data[i] * s;
    ...
    /// <param name="op2"></param>
    /// <returns></returns>
    public static bool operator ==(zvf4 op1, zvf4 op2)
    {
      for (int i = 0; i < op1.Length; i++)
